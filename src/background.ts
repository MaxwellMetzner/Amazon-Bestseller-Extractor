((): void => {
  ...
  const tabState = new Map<number, BsrTabState>();

  function isRecordValue(value: unknown): value is Record<string, unknown> {
    return typeof value === 'object' && value !== null;
  }

  function isUpdateBadgeRequest(message: unknown): message is UpdateBadgeRequest {
    return (
  ...
  function getSupportedAmazonHosts(): string[] {
  ...
  function registerActionRules(): void {
    const amazonHosts = getSupportedAmazonHosts();

    chrome.action.disable();

    chrome.declarativeContent.onPageChanged.removeRules(undefined, () => {
      const rules: chrome.events.Rule[] = amazonHosts.map((host) => ({
        conditions: [
  ...
  function setBadge(tabId: number, isVisible: boolean, lowestRank: number | null = null): void {
    if (isVisible) {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    sendResponse({ success: true } satisfies UpdateBadgeResponse);
  });
